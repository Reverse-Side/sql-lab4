...
class PaymentService:
    def __init__ (self, uow: IUnitOfWork):
        self.uow = uow
    async def process_payment(self, data: PaymentCreate, user_id: int) -> PaymentResponse:
        async with self.uow:
            ticket: Optional[TicketsORM] = await self.uow.tickets.find(id=data.ticket_id)
            if not ticket:
            ...
            if ticket.owner_id != user_id:
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You don't have access to this ticket")
    ...
    async def update_payment_status(self, payment_id: int, status_data: PaymentStatusUpdate) -> PaymentResponse:
        async with self.uow:
